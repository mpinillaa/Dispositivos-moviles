...
export class ContenidoPeliculaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.data.getPeliculas().subscribe( lista_peliculas=>{this.pelicula=lista_peliculas.peliculas.find(p=>p.id==numericId)}

      //lista_peliculas.peliculas=>{this.pelicula=lista_peliculas.peliculas.find(p=>p.id==numericId);   }
    );}}
